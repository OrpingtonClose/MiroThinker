...
#
# Prerequisites:
#   - MiroThinker repo at $REPO_ROOT (auto-detected from script location)
...
#   2. Adds MiroThinker/proxies to PYTHONPATH (observability + adaptive plugin)
...
# Resolve paths — agent code now lives inside MiroThinker
...
if [ ! -f "$STRANDS_AGENT_APP/main.py" ]; then
    ...
    echo "  Expected MiroThinker/strands-agent/main.py"
    exit 1
fi
...
# Add MiroThinker/proxies to PYTHONPATH so the agent can import
