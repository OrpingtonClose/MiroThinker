...
#   curl -fsSL https://raw.githubusercontent.com/OrpingtonClose/MiroThinker/main/scripts/deploy-production.sh | bash
#
...
# 4. Clone the repo
# ---------------------------------------------------------------------------
if [ ! -d /opt/MiroThinker/.git ]; then
    log "Cloning repo..."
    git clone https://github.com/OrpingtonClose/MiroThinker.git /opt/MiroThinker
else
    log "Repo already exists, pulling latest..."
    cd /opt/MiroThinker && git pull origin main
fi
log "Repo ready at /opt/MiroThinker."

# ---------------------------------------------------------------------------
# 5. Install Python dependencies
# ---------------------------------------------------------------------------
log "Installing Python dependencies..."
pip3 install -q -r /opt/MiroThinker/requirements.txt 2>/dev/null || true
...
cp /opt/MiroThinker/config/librechat.yaml /opt/LibreChat/librechat.yaml
# Docker Compose mounts from config/ dir, so also ensure it's there
cp /opt/MiroThinker/config/librechat.yaml /opt/MiroThinker/config/librechat.yaml.bak 2>/dev/null || true
