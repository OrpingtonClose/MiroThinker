#!/bin/bash
# =============================================================================
# MiroThinker — Master Startup Script
# Starts all services in order with health-check waits.
# Deploy to /opt/startup.sh on the VM.
# Set MIRO_ROOT to the MiroThinker checkout (default: /opt/MiroThinker).
# =============================================================================

set -euo pipefail

# Load environment variables if .env exists
MIRO_ROOT="${MIRO_ROOT:-/opt/MiroThinker}"
if [ -f /opt/.env ]; then
...
if [ "${SEARCH_BACKEND:-legacy}" = "mcp" ]; then
    if ! pgrep -f "litellm" > /dev/null; then
        screen -dmS litellm bash -c "litellm --config $MIRO_ROOT/config/litellm_config.yaml --port ${LITELLM_PORT:-4000} 2>&1 | tee /var/log/litellm.log"
...
if [ "${SEARCH_BACKEND:-legacy}" = "mcp" ]; then
    ...
    if ! pgrep -f "mcp_servers/exa/server.py" > /dev/null; then
        screen -dmS mcp-exa bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT && python3 -m mcp_servers.exa.server 2>&1 | tee /var/log/mcp_exa.log"
        echo "Exa MCP Server starting..."
    fi

    # --- Firecrawl MCP Server ---
    if ! pgrep -f "mcp_servers/firecrawl/server.py" > /dev/null; then
        screen -dmS mcp-firecrawl bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT && python3 -m mcp_servers.firecrawl.server 2>&1 | tee /var/log/mcp_firecrawl.log"
...
if ! pgrep -f "thinking_proxy.py" > /dev/null; then
    screen -dmS thinking-proxy bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && UPSTREAM_BASE='https://api.mistral.ai/v1' UPSTREAM_KEY=\"${MISTRAL_API_KEY:-}\" UPSTREAM_MODEL='mistral-large-latest' THINKING_PROXY_PORT=9100 python3 thinking_proxy.py 2>&1 | tee /var/log/thinking_proxy.log"
...
if ! pgrep -f "deep_research_proxy.py" > /dev/null; then
    screen -dmS deep-research bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && DEEP_RESEARCH_PORT=9200 python3 deep_research_proxy.py 2>&1 | tee /var/log/deep_research_proxy.log"
...
if ! pgrep -f "persistent_deep_research_proxy.py" > /dev/null; then
    screen -dmS persistent-research bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && PERSISTENT_RESEARCH_PORT=9300 python3 persistent_deep_research_proxy.py 2>&1 | tee /var/log/persistent_research_proxy.log"
...
if ! pgrep -f "miroflow_sprint_proxy.py" > /dev/null; then
    screen -dmS miroflow-sprint bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && MIROFLOW_SPRINT_PORT=9400 python3 miroflow_sprint_proxy.py 2>&1 | tee /var/log/miroflow_sprint_proxy.log"
...
if ! pgrep -f "swarm_proxy.py" > /dev/null; then
    screen -dmS swarm-proxy bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && UPSTREAM_BASE='https://api.venice.ai/api/v1' UPSTREAM_KEY=\"${VENICE_API_KEY:-${MISTRAL_API_KEY:-}}\" UPSTREAM_MODEL='venice-uncensored' SWARM_SYNTHESIS_MODEL='venice-uncensored' SWARM_WORKER_MODEL='venice-uncensored' SWARM_PROXY_PORT=9500 python3 swarm_proxy.py 2>&1 | tee /var/log/swarm_proxy.log"
...
if ! pgrep -f "godmode_proxy.py" > /dev/null; then
    screen -dmS godmode-proxy bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && GODMODE_PROXY_PORT=9600 python3 godmode_proxy.py 2>&1 | tee /var/log/godmode_proxy.log"
...
if ! pgrep -f "xai_native_proxy.py" > /dev/null; then
    screen -dmS xai-native-proxy bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && XAI_PROXY_PORT=9700 python3 xai_native_proxy.py 2>&1 | tee /var/log/xai_native_proxy.log"
...
if ! pgrep -f "tier_chooser_proxy.py" > /dev/null; then
    screen -dmS tier-chooser bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && TIER_CHOOSER_PORT=9900 python3 tier_chooser_proxy.py 2>&1 | tee /var/log/tier_chooser_proxy.log"
...
if [ -z "$VENICE_API_KEY" ]; then
    ...
    screen -dmS heretic-proxy bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && HERETIC_PROXY_PORT=9950 python3 heretic_proxy.py 2>&1 | tee /var/log/heretic_proxy.log"
...
if [ -z "$VENICE_API_KEY" ]; then
    ...
    screen -dmS miro-proxy bash -c "set -a; source /opt/.env 2>/dev/null; set +a; cd $MIRO_ROOT/proxies && MIRO_PROXY_PORT=9951 python3 miro_proxy.py 2>&1 | tee /var/log/miro_proxy.log"
