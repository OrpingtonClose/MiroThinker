...
# ---------------------------------------------------------------------------
# Source tagging tests (cumulative runs)
# ---------------------------------------------------------------------------

class TestSourceTagging:
    """Findings should carry source_model and source_run when provided."""

    def test_finding_has_source_tags(self, store):
        tagged_tools = build_worker_tools(
            store=store,
            worker_angle="insulin_timing",
            worker_id="tagged_worker",
            phase="test_wave_1",
            source_model="qwen3.5-32b-abl",
            source_run=3,
        )
        store_finding = tagged_tools[2]
        store_finding(
            fact="Tagged finding for cumulative run test",
            confidence=0.85,
        )
        with store._lock:
            rows = store.conn.execute(
                "SELECT source_model, source_run FROM conditions "
                "WHERE fact LIKE '%Tagged finding%' AND row_type = 'finding'"
            ).fetchall()
        assert len(rows) == 1
        assert rows[0][0] == "qwen3.5-32b-abl"
        assert rows[0][1] == 3

    def test_finding_without_source_tags(self, store):
        untagged_tools = build_worker_tools(
            store=store,
            worker_angle="insulin_timing",
            worker_id="untagged_worker",
            phase="test_wave_1",
        )
        store_finding = untagged_tools[2]
        store_finding(
            fact="Untagged finding for baseline test",
            confidence=0.7,
        )
        with store._lock:
            rows = store.conn.execute(
                "SELECT source_model, source_run FROM conditions "
                "WHERE fact LIKE '%Untagged finding%' AND row_type = 'finding'"
            ).fetchall()
        assert len(rows) == 1
        assert rows[0][0] == ""
        assert rows[0][1] == 0

    def test_extra_tools_appended(self, store):
        def dummy_tool() -> str:
            return "dummy"

        tools_with_extra = build_worker_tools(
            store=store,
            worker_angle="insulin_timing",
            worker_id="extra_worker",
            phase="test",
            extra_tools=[dummy_tool],
        )
        # 6 built-in + 1 extra
        assert len(tools_with_extra) == 7
        assert tools_with_extra[-1] is dummy_tool
